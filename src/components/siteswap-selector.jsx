...
export default function SiteswapSelector({clickHandler}) {
    ...
    return <div className="flex flex-row items-center justify-content">
            <input value={input} placeholder={"Enter siteswap"} className="w-full p-2 h-10 bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-md" onChange={(event) => setInput(event.target.value)}/>
            <button className='bg-blue-400 text-lg text-black px-4 py-2 rounded-lg mx-2' onClick={() => {clickHandler(input); setInput("")}}>Go!</button>
